...
from ..audio.recorder import AudioRecorder, AudioError
from ..config.manager import ConfigManager, ConfigError
from ..services.clipboard import ClipboardService, ClipboardError
from ..speech.enhancer import TextEnhancer, EnhancementError
from ..speech.transcriber import SpeechTranscriber, TranscriptionError
from ..ui.overlay import UIManager, THEME
...
class STTOrchestrator:
    ...
    def run_transcription(self, profile: Optional[str] = None, no_llm: bool = False) -> None:
    ...
    def _start_model_loading_monitor(self) -> None:
        ...
        def check_model_loading():
            while (
                ...
                if (
                    self.transcriber._model_loaded.is_set() 
                    and self.transcriber.model
                ):
    ...
    def _process_audio(self, audio_file: Path, profile: Optional[str], no_llm: bool) -> None:
        ...
        try:
            ...
            self.ui_manager.set_status("⏳ Waiting for model...", THEME["accent_yellow"])
            timeout = self.config.get("whisper.load_timeout", 60)
            if not self.transcriber.wait_for_model(timeout):
                print("❌ Failed to load Whisper model")
                self.ui_manager.set_status("❌ Model loading failed", THEME["accent_red"])
                return
            ...
            if llm_enabled:
                ...
                if profile:
                    ...
                    self.ui_manager.set_status(f"🔄 Refining ({profile})...", THEME["accent_purple"])
                else:
                    logger.info("Enhancing text with LLM using default profile")
                    print("🔄 Refining text...")
                    self.ui_manager.set_status("🔄 Refining text...", THEME["accent_purple"])
            ...
            if self.config.get("clipboard.auto_paste", False):
                ...
                self.ui_manager.set_status("✅ Copied to clipboard!", THEME["accent_green"])
