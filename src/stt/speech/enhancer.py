...
class TextEnhancer:
    ...
    def _call_openai(self, prompt: str, text: str) -> Optional[str]:
        ...
        try:
            ...
            if not api_key:
                logger.error(
                    "OpenAI API key not found. Check .env file or environment variables."
                )
            ...
            if enhanced_text:
                logger.debug("OpenAI enhancement successful")
    ...
    def _call_anthropic(self, prompt: str, text: str) -> Optional[str]:
        ...
        try:
            ...
            if not api_key:
                logger.error(
                    "Anthropic API key not found. Check .env file or environment variables."
                )
            ...
            if enhanced_text:
                logger.debug("Anthropic enhancement successful")
                return enhanced_text.strip()
